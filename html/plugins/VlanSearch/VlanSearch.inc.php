...
if (! Auth::user()->hasGlobalRead()) {
    ...
    $whereDev = ' AND `devices`.`device_id` IN ' . dbGenPlaceholders(count($device_ids));
    $whereVlan = ' WHERE `vlans`.`device_id` IN ' . dbGenPlaceholders(count($device_ids));
    ...
    foreach ($result as $device_id => $device_data) {
        ...
        foreach ($device_data['vlan'] as $vlan_data) {
            ...
            foreach ($vlan_data['ports'] as $port_id => $port_data) {
                ...
                $rowPName = (strtoupper($ifName) != strtoupper($ifAlias)) ? $ifName . '</br>' . $ifAlias : $ifName;
