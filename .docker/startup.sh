...
### => snmpd
DF="/etc/default/snmpd";
echo "SNMPDOPTS='-LS5d -Lf /dev/null -u root -g root -I -p /run/snmpd.pid'" > $DF
chown root:root ${DF}
...
### => enabled services
[ "${NMS_ROLE}" = 'poller' ] && echo "cron snmpd snmptrapd rsyslog" > /tmp/services
[ "${NMS_ROLE}" = 'normal' ] && echo "cron snmpd snmptrapd rsyslog php-fpm" > /tmp/services
[ "${NMS_ROLE}" = 'devel' ] && echo "cron snmpd snmptrapd rsyslog php-fpm ssh" > /tmp/services
