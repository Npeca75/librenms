...
if ($sType == 'ipv6' && ! empty($route)) {
    ...
    $route = rtrim(rtrim($route6->uncompressed(), ':0000'), '0');
}

$query = ' FROM `route` AS R, `ports` AS P, `devices` AS D';
$query .= " WHERE P.port_id = R.port_id AND D.device_id = R.device_id AND inetCidrRouteDestType='$sType' $where";

if (! empty($route)) {
    $query .= ' AND inetCidrRouteDest LIKE ?';
    $param[] = '%' . $route . '%';
}
if (! empty($prefix)) {
    $query .= " AND inetCidrRoutePfxLen='$prefix'";
}
...
$count_sql = "SELECT COUNT(`route_id`) $query";
...
if ($rowCount != -1) {
    $query .= " LIMIT $limit_low,$limit_high";
}

$query = "SELECT *,`P`.`ifDescr` AS `interface`, `R`.`inetCidrRouteDest` $query";
$fromDB = array_map(fn ($x) => (array) $x, DB::select($query, $param));
