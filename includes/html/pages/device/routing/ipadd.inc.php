...
foreach (['ipv4', 'ipv6'] as $adType) {
    ...
    $sort = ($adType == 'ipv4') ? 'INET_ATON(' : 'INET6_ATON(';
    $sort = $sort . $adType . '_addresses.' . $selCol . ')';
    $query = '
        SELECT
            ports.port_id, ports.ifName, ports.ifAlias, ports.ifOperStatus, ports.ifAdminStatus, '
            . $adType . '_addresses.' . $selCol . ', ' . $adType . '_addresses.' . $adType . '_prefixlen
        FROM
            ports, ' . $adType . '_addresses, devices
        WHERE
            ports.device_id = devices.device_id AND ' . $adType . '_addresses.port_id = ports.port_id AND devices.device_id = ?
        ORDER BY
            ports.ifName, ' . $sort;

    $fromDB = array_map(fn ($x) => (array) $x, DB::select($query, [$device['device_id']]));
