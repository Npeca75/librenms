...
$sIndex = array_flip($scripts['MIKROTIK-MIB::mtxrScriptName'] ?? [])['LNMS_wginfo'] ?? null;
if (isset($sIndex)) {
    echo "Mikrotik WG info \n";
    $data = SnmpQuery::get('MIKROTIK-MIB::mtxrScriptRunOutput.' . $sIndex)->value();
    $interface_client_map = json_decode(stripslashes((string) $data), true)['data'];
    d_echo(json_last_error());
    d_echo($parsed_json);
}
